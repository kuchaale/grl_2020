...
def open_infile(infiles):
    if isinstance(infiles, str) & ('zarr' in infiles):
        ...
        ds = xr.open_mfdataset(infiles, concat_dim = 'time', coords='minimal')
    return ds
...
if var in ['uas', 'vas', 'pr']:
    hourly_index = pd.date_range('1979-01-01-06', '2011-01-01', freq='6H')
elif var in ['lwatend']:
...
qbo_phase = sys.argv[5]
type_ls = ['himalayas']#, 'westamer', 'eastasia']

if zarr:
    infiles = '{}{}{}_19790101-20101231.zarr'.format(cesta,var,lev_sys_fi)
    print(infiles)
else:
...
with open_infile(infiles) as ds:
    with xr.open_dataset( cesta+var+'/'+lev_sys_fo+var+'_climatology'+w_clim+'.nc') as clim:
        ...
        for di, ssw_type in enumerate(type_ls):
            ...
            df_dates = open_date_file('../notebooks/accelogw_{}_hotspot@{}hPa_{}dayts_{}indexes.csv'.format(ssw_type, ch_lev, timescale, qbo_phase))
        #sys.exit()
            xa_ls = []
            pv_ls = []
            ...
            outfile = "{}composites{}/{}/{}{}_{}_comp_{}_{}days.nc".format(cesta, w_clim, qbo_phase, var, lev_sys_fi, what, type_ls[di], timescale)
